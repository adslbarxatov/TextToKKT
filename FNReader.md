...
находятся в тестировании и могут работать неправильно*;
...
3. Обработка фискальных данных:
    ...
    - Получение полного отчёта по всем документам, хранящимся в ФН;
...
- ***Формирование выгрузки .fnc с последующей её загрузкой***. Работает аналогично первому варианту,
но не позволяет просматривать полный набор реквизитов документов.
...
Здесь необходимо выбрать порт, который соответствует устройству, появившемуся в системе
...
Запрашивает статус ФН и, если активен флажок «***и прочитать***» (будет иметь жёлтый цвет), запускает процесс
...
Если процесс был прерван, или флажок «***и прочитать***» не был выставлен, аналитика будет недоступна, но станут активными функции
обмена с ОФД, прямого чтения и записи в ФН.
...
- Эталонный формат архива, создаваемый этой программой, а также программами FNArc, FNRun и FNTransfer (```.fnc```). *На данный
  момент может работать некорректно с выгрузками в ФФД 1.1 и 1.2*.
...
Запускает отправку документов из ФН оператору фискальных данных, если очередь отправки не пуста. *Для ФФД 1.1 и 1.2
...
избавить пользователя от необходимости ручной настройки соединения. Программе известны настройки связи со всеми ОФД, действующими на 1 февраля 2022 года.
...
доступны функции чтения статуса ФН, а также чтения отдельного документа. Кнопки полного считывания ФН, обмена с ОФД, формирования дампов и аналитического
функционала будут неактивны. Полнота считывания данных в этом режиме также определяется состоянием переключателя «***детали***».
...
Позволяет отправить содержимое поля статуса на печать. Поддерживает флажок «***+ статус ФН***».
...
Для сохранения используется кнопка «В файл» напротив списка отчётов.
...
Кроме того, станут доступны некоторые ***интерактивные функции***, которые предполагают отображение сведений в текстовом поле в середине окна:
